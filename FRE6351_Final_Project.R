...
res = nff(x = data_seasonality + 2, n = 25L, add = TRUE, up = 100L, col = 2L)
colors = rainbow(18L, alpha = 0.3)
nff(x = y, n = 18L, up = 100L, col = colors[1])
for(i in 1:18){
  ad = ifelse(i == 1, FALSE, TRUE)
  nff(x = data_seasonality + 2, n = i, up = 100L, col = colors[i], add = ad, main = "All waves up to 18th harmonic")
}
